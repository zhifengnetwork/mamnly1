...
class Index extends ApiBase
{
	...
	public function index(){
		...
		//头条
		$articlelist = M('article')->field('article_id,title,link')->where(['cat_id'=>$cat_id,'is_open'=>1])->order('add_time desc')->limit('0,3')->select();
		...
		$hot_goods = M('goods')->where("is_hot=1 and is_on_sale=1")->order('goods_id DESC')->limit(2)->select();//首页热卖商品
